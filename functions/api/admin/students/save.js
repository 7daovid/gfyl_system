...
export async function onRequestPost(context) {
  ...
  if (phone && !/^[0-9+\-\s]{5,20}$/.test(phone)) throw new ApiError('联系方式格式不正确', 400);
  ...
  if (id) {
    ...
    await env.DB.prepare("UPDATE students SET student_no = ?, name = ?, dept = ?, phone = ?, reg_status = CASE WHEN ? = 1 THEN 'approved' ELSE reg_status END, active = ? WHERE id = ?")
      .bind(no, name, dept, phone, active, active, id).run();
    ...
    await writeLog(env, {
      ...
      targetType: 'student',
      targetId: id,
      oldValue: old.student_no + ' / ' + old.name + ' / ' + (old.dept || '-') + ' / 电话:' + (old.phone || '-') + ' / ' + (Number(old.active) === 1 ? '启用' : '停用'),
      newValue: no + ' / ' + name + ' / ' + (dept || '-') + ' / 电话:' + (phone || '-') + ' / ' + (active === 1 ? '启用' : '停用'),
  ...
  const res = await env.DB.prepare(
    'INSERT INTO students (student_no, name, dept, phone, reg_status, active, created_ms, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)'
  ).bind(no, name, dept, phone, 'approved', active, now, bjTime(now)).run();
